...
// Add a new event
export const addEvent = (dateKey: string, title: string, startTime?: number, endTime?: number, color: ColorName = "gray") =>
  Effect.gen(function* () {
    const store = yield* Ref.get(eventStoreRef)
...
export const updateEvent = (eventId: string, updates: Partial<Omit<CalendarEvent, "id">>) =>
  Effect.gen(function* () {
    const store = yield* Ref.get(eventStoreRef)
    const newStore = new Map(store)
    ...
    for (const [dateKey, events] of newStore.entries()) {
      ...
      if (eventIndex !== -1 && event) {
        ...
        if (updates.date && updates.date !== dateKey) {
          ...
          // Add to new date
          const targetEvents = [...(newStore.get(updates.date) ?? []), updatedEvent]
