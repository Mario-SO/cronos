...
export const startGoogleOAuth = () =>
	Effect.gen(function* () {
		...
		if (!tokenResponse.ok) {
			const errorText = yield* Effect.tryPromise(() => tokenResponse.text());
			return yield* Effect.fail(
				new Error(`Token exchange failed: ${errorText}`),
			);
		}
...
export const refreshGoogleToken = (settings: GoogleSettings) =>
	Effect.gen(function* () {
		if (!settings.refreshToken) {
			return yield* Effect.fail(new Error("Missing refresh token"));
		}
		...
		if (!response.ok) {
			...
			return yield* Effect.fail(
				new Error(`Token refresh failed: ${errorText}`),
			);
		}
...
export function googleApiRequest<T>(
	accessToken: string,
	path: string,
	options: RequestInit = {},
	...
	return Effect.gen(function* () {
		...
		return (yield* Effect.tryPromise(() => response.json())) as T;
	});
}
